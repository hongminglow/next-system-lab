import { cacheLife } from "next/cache";
import { Suspense } from "react";

async function CachedContent() {
	"use cache";
	cacheLife({ stale: 10, revalidate: 10, expire: 60 });
	const generatedAt = new Date().toISOString();
	return (
		<div className="rounded-md border border-black/10 p-4 text-sm dark:border-white/15">
			<span className="font-medium">generatedAt:</span> {generatedAt}
		</div>
	);
}

export default function Page() {
	return (
		<main className="mx-auto flex min-h-screen w-full max-w-2xl flex-col gap-4 px-6 py-10">
			<h1 className="text-xl font-semibold tracking-tight">D2) ISR-style (cacheLife revalidate=10)</h1>
			<p className="text-sm text-zinc-700 dark:text-zinc-300">
				Cached for ~10s then revalidated on subsequent requests.
			</p>

			<Suspense fallback={<div className="text-sm">Loading…</div>}>
				<CachedContent />
			</Suspense>

			<a className="text-sm underline" href="/tests/cms">
				Back
			</a>
		</main>
	);
}
